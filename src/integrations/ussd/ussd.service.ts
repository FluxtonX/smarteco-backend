...
export class UssdService {
    ...
    async processRequest(
        ...
        this.logger.log(`USSD [${sessionId}] Phone: ${phone}, Level: ${level}, Input: "${text}"`);
    ...
    private async handleSchedulePickup(
        ...
        if (level === 3) {
            const wasteTypes = ['ORGANIC', 'RECYCLABLE', 'EWASTE', 'GENERAL', 'HAZARDOUS'];
            const timeSlots = ['MORNING_8_10', 'MORNING_10_12', 'AFTERNOON_2_4', 'AFTERNOON_4_6'];
            const wasteLabels = ['Organic', 'Recyclable', 'E-Waste', 'General', 'Hazardous'];
            const timeLabels = ['8-10 AM', '10-12 PM', '2-4 PM', '4-6 PM'];

            const wasteIdx = parseInt(inputs[1]) - 1;
            const timeIdx = parseInt(inputs[2]) - 1;

            if (wasteIdx < 0 || wasteIdx >= wasteTypes.length || timeIdx < 0 || timeIdx >= timeSlots.length) {
