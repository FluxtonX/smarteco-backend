...
export class UssdService {
  ...
  private async handleSchedulePickup(
    ...
    if (level === 3) {
      ...
      if (!user) {
        return 'END You are not registered. Please download the SmartEco AI app to register first.';
      }
  ...
  private async handleCheckStatus(phone: string): Promise<string> {
    const user = await this.prisma.user.findUnique({ where: { phone } });
    if (!user) {
      return 'END You are not registered. Download the SmartEco AI app to register.';
    }
