...
export class WhatsAppService {
    ...
    constructor(
        ...
        this.logger.log(`WhatsApp service initialized (configured: ${this.isConfigured})`);
    }

    // ─── SEND WHATSAPP MESSAGE ──────────────────────

    async sendMessage(to: string, body: string): Promise<{ sid: string; status: string }> {
        if (!this.isConfigured) {
            this.logger.warn(`WhatsApp not configured — mock sending to ${to}: "${body.substring(0, 50)}..."`);
        ...
        try {
            ...
            this.logger.error(`WhatsApp send failed: ${error.message}`, error.response?.data);
            throw error;
        }
    }
    ...
    async processIncoming(
        ...
        try {
            ...
            // Handle commands
            if (input === 'hi' || input === 'hello' || input === 'menu' || input === 'start') {
    ...
    async sendPickupConfirmation(phone: string, reference: string, wasteType: string, date: string) {
        return this.sendMessage(phone, this.buildResponse([
            '✅ *Pickup Scheduled!*',
            '',
            `📋 Reference: ${reference}`,
    ...
    async sendCollectorAssigned(phone: string, reference: string, collectorName: string) {
        return this.sendMessage(phone, this.buildResponse([
            '👤 *Collector Assigned!*',
            '',
            `📋 Pickup: ${reference}`,
    ...
    async sendPickupCompleted(phone: string, reference: string, points: number, weight: number) {
        return this.sendMessage(phone, this.buildResponse([
            '🎉 *Pickup Completed!*',
            '',
            `📋 Reference: ${reference}`,
    ...
    private async getBinStatus(userId: string): Promise<string> {
        ...
        bins.forEach((bin) => {
            const icon = bin.fillLevel >= 80 ? '🔴' : bin.fillLevel >= 50 ? '🟡' : '🟢';
            const bar = '▓'.repeat(Math.round(bin.fillLevel / 10)) + '░'.repeat(10 - Math.round(bin.fillLevel / 10));
