...
export class MomoService {
    ...
    constructor(private readonly configService: ConfigService) {
        this.baseUrl = this.configService.get<string>('MOMO_BASE_URL') || 'https://sandbox.momodeveloper.mtn.com';
    ...
    async requestToPay(
        ...
        if (!this.apiKey || !this.subscriptionKey) {
            this.logger.warn('MoMo API keys not configured — returning sandbox mock response');
            return {
                referenceId,
                status: 'PENDING',
                externalId,
        ...
        try {
            ...
            this.logger.log(`MoMo Request to Pay initiated: ${referenceId} for ${amount} ${currency}`);
            ...
            this.logger.error(`MoMo Request to Pay failed: ${error.message}`, error.response?.data);
            throw error;
        }
    }
    ...
    async checkPaymentStatus(referenceId: string): Promise<MomoPaymentStatusResult> {
        // If no API keys configured, return sandbox mock
        if (!this.apiKey || !this.subscriptionKey) {
            this.logger.warn('MoMo API keys not configured — returning sandbox mock status');
            return {
                referenceId,
                status: 'SUCCESSFUL',
                amount: '0',
        ...
        try {
            ...
            this.logger.log(`MoMo payment ${referenceId} status: ${response.data.status}`);
            ...
            this.logger.error(`MoMo check status failed: ${error.message}`, error.response?.data);
            throw error;
        }
    }
    ...
    async disbursement(
        ...
        if (!this.apiKey || !this.subscriptionKey) {
            this.logger.warn('MoMo API keys not configured — returning sandbox mock disbursement');
        ...
        try {
            ...
            this.logger.log(`MoMo Disbursement initiated: ${referenceId} for ${amount} ${currency}`);
            return { referenceId, status: 'PENDING' };
        } catch (error: any) {
            this.logger.error(`MoMo Disbursement failed: ${error.message}`, error.response?.data);
            throw error;
        }
    }
