...
export class AirtelService {
    ...
    constructor(private readonly configService: ConfigService) {
        this.baseUrl = this.configService.get<string>('AIRTEL_BASE_URL') || 'https://openapiuat.airtel.africa';
        ...
        this.logger.log(`Airtel Money service initialized (sandbox: ${this.isSandbox})`);
    }
    ...
    async requestToPay(
        ...
        if (!this.clientId || !this.clientSecret) {
            this.logger.warn('Airtel API keys not configured — returning sandbox mock response');
            return {
                referenceId,
                status: 'PENDING',
            };
        ...
        try {
            ...
            this.logger.error(`Airtel Request to Pay failed: ${error.message}`, error.response?.data);
            throw error;
        }
    }
    ...
    async checkPaymentStatus(transactionId: string): Promise<AirtelPaymentStatusResult> {
        // If no keys configured, return sandbox mock
        if (!this.clientId || !this.clientSecret) {
            this.logger.warn('Airtel API keys not configured — returning sandbox mock status');
            return {
        ...
        try {
            ...
            this.logger.error(`Airtel check status failed: ${error.message}`, error.response?.data);
            throw error;
        }
    }
    ...
    async disbursement(
        ...
        if (!this.clientId || !this.clientSecret) {
            this.logger.warn('Airtel API keys not configured — returning sandbox mock disbursement');
        ...
        try {
            ...
            this.logger.log(`Airtel Disbursement initiated: ${referenceId} for ${amount} ${currency}`);
            return { referenceId, status: 'PENDING' };
        } catch (error: any) {
            this.logger.error(`Airtel Disbursement failed: ${error.message}`, error.response?.data);
            throw error;
        }
    }
