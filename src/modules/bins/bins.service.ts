...
export class BinsService {
    ...
    async updateFillLevel(userId: string, binId: string, dto: UpdateFillLevelDto) {
        const bin = await this.prisma.bin.findUnique({
            where: { id: binId },
        });
        ...
        if (dto.fillLevel >= BIN_ALERT_THRESHOLD && dto.fillLevel < BIN_AUTO_SCHEDULE_THRESHOLD) {
            alertTriggered = true;
            this.logger.log(
                `Alert: Bin ${bin.qrCode} is ${dto.fillLevel}% full`,
            );
        }
    ...
    async scanBin(dto: ScanBinDto) {
        ...
        return {
            ...
            data: {
                ...
                owner: {
                    id: bin.user.id,
                    name: `${bin.user.firstName || ''} ${bin.user.lastName || ''}`.trim() || 'Unknown',
