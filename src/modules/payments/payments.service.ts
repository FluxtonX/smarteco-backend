...
export class PaymentsService {
    ...
    async initiatePayment(userId: string, dto: InitiatePaymentDto) {
        ...
        const method = dto.method === PaymentMethodEnum.MOMO
        ...
        if (payment.status === PaymentStatus.PENDING && payment.externalRef) {
            try {
                ...
                if (payment.method === PaymentMethod.MTN_MOMO) {
                    const result = await this.momoService.checkPaymentStatus(payment.externalRef);
                    providerStatus = result.status;
                } else {
                    const result = await this.airtelService.checkPaymentStatus(payment.externalRef);
    ...
    async handleWebhook(provider: string, body: any) {
        this.logger.log(`Payment webhook received from ${provider}: ${JSON.stringify(body)}`);
        ...
        if (payment.status !== PaymentStatus.PENDING) {
            this.logger.log(`Payment ${transactionRef} already processed: ${payment.status}`);
            return { received: true };
        }

        const newStatus = status === 'SUCCESSFUL' || status === 'SUCCESS'
        ...
        this.logger.log(`Payment ${transactionRef} updated to ${newStatus} via webhook`);
